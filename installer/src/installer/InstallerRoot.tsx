...
import { getFormComponent, getStateDisplayName, hasCustomForm } from "./stateToFormMap";
...
const InstallerRoot: Component<InstallerRootProps> = (props) => {
  ...
  const getProgressInfo = () => {
    ...
    const progress = Math.max(0, Math.min(100, (currentIndex / (allStates.length - 1)) * 100));
      ...
      {import.meta.env.DEV && (
        <div class="mt-8 p-4 bg-gray-100 rounded-lg">
          ...
          <div class="text-xs text-gray-600 space-y-1">
            <div><strong>Current State:</strong> {currentStateName()}</div>
            <div><strong>Has Custom Form:</strong> {hasCustomForm(currentStateName()) ? "Yes" : "No"}</div>
            <div><strong>Progress:</strong> {progressInfo.progress.toFixed(1)}%</div>
            <div><strong>Error:</strong> {state.context.error ? state.context.error.toString() : "None"}</div>
          </div>
        </div>
      )}
    </div>
