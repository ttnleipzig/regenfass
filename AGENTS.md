...
- **Web**: pnpm workspace (`pnpm-workspace.yaml` → `web/*`). Primary apps:
  ...
  - `web/homepage` — `@ttnleipzig/regenfass-homepage`
...
- Every new or updated project `README.md` must include a suitable official Regenfass banner at the top. Generate or select the matching banner from the Brand repository before completing the documentation change.
...
- **listmonk email templates**: whenever an email template is changed in listmonk, update the corresponding copy in `web/templates/email/` in the same change. Keep repository changes and the live listmonk templates synchronized in both directions; after changing repository copies, synchronize the active Listmonk templates immediately.
...
- **What must run for installer web work:** only the installer Vite app (`pnpm dev:installer` → **5173**). `@regenfass/brand` is a workspace library (no separate server). Dashboard/Postgres (`web/dashboard`), PlatformIO firmware, homepage/docs/playground are optional for installer UI work.
- **Other web apps (optional):** `pnpm dev:playground` → 5177, `pnpm dev:homepage` → 5175, `pnpm dev:docs` → 5176.
