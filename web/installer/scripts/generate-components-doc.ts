...
class ComponentDocGenerator {
  ...
  private get componentRoots(): string[] {
    return [
      this.componentsRoot,
      path.resolve(process.cwd(), '../brand/src/components'),
    ];
  }
  ...
  private async discoverComponents(): Promise<string[]> {
    const files: string[] = [];
    for (const root of this.componentRoots) {
      const pattern = path.join(root, '**/*.{ts,tsx}');
  ...
  private extractComponentInfo(sourceFile: SourceFile): ComponentInfo[] {
    ...
    const relativePath = path.relative(
      this.componentRoots.find((root) => filePath.startsWith(root)) ?? this.componentsRoot,
      filePath,
    );
...
*Generated on ${new Date().toLocaleString('en-US', { 
  ...
  private generateComponentMarkdown(component: ComponentInfo): string {
    ...
    markdown += `**Import:**\n\n\`\`\`typescript\nimport { ${component.name} } from '@regenfass/brand';\n\`\`\`\n\n`;
